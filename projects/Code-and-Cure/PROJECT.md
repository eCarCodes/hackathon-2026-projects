...
1. **Symptom triage.** Patient types free text. The system suggests a specialty based on key words (NO DIAGNOSIS, ONLY RECCOMENDATION).
...
The patient sees the recommendation and can accept it or override it. We deliberately did not auto-route — clients confirm.
...
The patient picks a slot. The booking record gets written to Supabase with patient ID, doctor ID, time, and status. Simply, CRUD with proper constraints. This was the easiest pillar to build and the one we touched the least over the weekend.

Step 4 — Doctor signs in and runs the encounter

The doctor logs in and sees their schedule. They open a booked appointment, run the visit (in real life — for the demo, we pre-recorded a consultation), and trigger SOAP generation.
...
This is the gate. The doctor sees the parsed SOAP, edits anything the model got wrong (or anything they want to change for any reason), and clicks approve. The note's state moves from `draft` to `approved` (ensures the human interaction in the loop).
